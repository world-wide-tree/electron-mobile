...
class ApiClient {
  ...
  ApiClient(this._client, this._localDataSource);

  dynamic get(String path, {Map<dynamic, dynamic>? params}) async {
    String sessionId =
        await _localDataSource.getSessionId() ?? "";
  ...
  dynamic postphoto(String path, {required String filename}) async {
    ...
    if (kDebugMode) {
      print("Zapros nachalo ${Uri.parse('${ApiConstants.baseApiUrl}$path')}");
    }
    request.files.add(await http.MultipartFile.fromPath('avatar', filename));

    if (kDebugMode) {
      print("Zapros nachalo 3");
    }
    request.headers.addAll(headers);

    http.StreamedResponse response = await request.send();
    if (kDebugMode) {
      print("Zapros: ${response.statusCode}");
    }
  ...
  dynamic post(
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      log("Status code ${response.statusCode} full path $pth");
      throw Exception(response.reasonPhrase);
    }
  }
